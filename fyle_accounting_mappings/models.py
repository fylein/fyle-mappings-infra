...
def validate_mapping_settings(mappings_settings: List[Dict]):
    ...
    for mappings_setting in mappings_settings:
        if ('source_field' not in mappings_setting) and (
            not mappings_setting['source_field']
        ):
            bulk_errors.append(
                {'row': row, 'value': None, 'message': 'source field cannot be empty'}
            )

        if ('destination_field' not in mappings_setting) and (
            not mappings_setting['destination_field']
        ):
            bulk_errors.append(
                {
                    'row': row,
...
def create_mappings_and_update_flag(
    mapping_batch: list, set_auto_mapped_flag: bool = True, **kwargs
):
    ...
    if set_auto_mapped_flag:
        ...
        for mapping in mappings:
            expense_attributes_to_be_updated.append(
                ExpenseAttribute(
                    id=(
                        mapping.source_category.id
                        if model_type == CategoryMapping
                        else mapping.source.id
                    ),
                    auto_mapped=True,
                )
            )
...
def construct_mapping_payload(
    employee_source_attributes: list,
    employee_mapping_preference: str,
    destination_id_value_map: dict,
...
class ExpenseAttribute(models.Model):
    ...
    def bulk_update_deleted_expense_attributes(attribute_type: str, workspace_id: int):
        ...
        for attribute in deleted_attributes:
            attributes_to_be_updated.append(
                ExpenseAttribute(
                    id=attribute.id, active=False, updated_at=datetime.now()
                )
            )
    ...
    def bulk_create_or_update_expense_attributes(
        ...
        for attribute in attributes:
            if (
                attribute['value'] not in existing_attribute_values
                and attribute['value'] not in values_appended
            ):
                ...
                if update:
                    attributes_to_be_updated.append(
                        ExpenseAttribute(
                            ...
                            source_id=attribute['source_id'],
                            detail=(
                                attribute['detail'] if 'detail' in attribute else None
                            ),
                            active=(
                                attribute['active'] if 'active' in attribute else None
                            ),
                        )
                    )
        ...
        if attributes_to_be_updated:
            ExpenseAttribute.objects.bulk_update(
                attributes_to_be_updated,
                fields=['source_id', 'detail', 'active'],
                batch_size=50,
            )
...
class DestinationAttribute(models.Model):
    ...
    def create_or_update_destination_attribute(attribute: Dict, workspace_id):
        ...
        destination_attribute, _ = DestinationAttribute.objects.update_or_create(
            ...
            defaults={
                ...
                'detail': attribute['detail'] if 'detail' in attribute else None,
                'code': (
                    ' '.join(attribute['code'].split())
                    if 'code' in attribute and attribute['code']
                    else None
                ),
            },
        )
    ...
    def bulk_create_or_update_destination_attributes(
        ...
        existing_attributes = DestinationAttribute.objects.filter(**filters).values(
        ...
        for attribute in attributes:
            if (
                attribute['destination_id'] not in existing_attribute_destination_ids
                ...
                if (
                    ...
                    and is_import_to_fyle_enabled
                    and (
                        (
                            attribute['value']
                            and primary_key_map[attribute['destination_id']]['value']
                            and attribute['value'].lower()
                            != primary_key_map[attribute['destination_id']][
                                'value'
                            ].lower()
                        )
                        or (
                            ...
                            and attribute['code']
                            and attribute['code']
                            != primary_key_map[attribute['destination_id']]['code']
                        )
                    )
                ):
                ...
                if update and (
                    ...
                    or (
                        ...
                        != primary_key_map[attribute['destination_id']]['detail']
                    )
                    or (
                        'active' in attribute
                        and attribute['active']
                        != primary_key_map[attribute['destination_id']]['active']
                    )
                    or (
                        'code' in attribute
                        and attribute['code']
        ...
        if attributes_to_be_updated:
            DestinationAttribute.objects.bulk_update(
                attributes_to_be_updated,
                fields=['detail', 'value', 'active', 'updated_at', 'code'],
                batch_size=50,
            )
...
class ExpenseField(models.Model):
    ...
    @staticmethod
    def create_or_update_expense_fields(
        attributes: List[Dict], fields_included: List[str], workspace_id
    ):
        '''
        ...
        for expense_field in attributes:
            if (
                expense_field['field_name'] in fields_included
                or expense_field['type'] == 'DEPENDENT_SELECT'
            ):
                expense_fields, _ = ExpenseField.objects.update_or_create(
                    attribute_type=expense_field['field_name']
                    .replace(' ', '_')
                    .upper(),
                    workspace_id=workspace_id,
                    defaults={
                        ...
                        'is_enabled': (
                            expense_field['is_enabled']
                            if 'is_enabled' in expense_field
                            else False
                        ),
                    },
                )
...
class MappingSetting(models.Model):
    ...
    expense_field = models.ForeignKey(
        ...
        help_text='Reference to Expense Field model',
        related_name='expense_fields',
        null=True,
    )
    workspace = models.ForeignKey(
        Workspace,
        ...
        with transaction.atomic():
            for setting in settings:
                ...
                mapping_setting, _ = MappingSetting.objects.update_or_create(
                    ...
                    expense_field_id=(
                        setting['parent_field'] if 'parent_field' in setting else None
                    ),
                    defaults={
                        'import_to_fyle': (
                            setting['import_to_fyle']
                            if 'import_to_fyle' in setting
                            else False
                        ),
                        'is_custom': (
                            setting['is_custom'] if 'is_custom' in setting else False
                        ),
                    },
                )
...
class Mapping(models.Model):
    ...
    def create_or_update_mapping(
        ...
        '''
        settings = MappingSetting.objects.filter(
    ...
    def bulk_create_mappings(
        ...
        destination_type: str,
        workspace_id: int,
        set_auto_mapped_flag: bool = True,
    ):
        '''
        ...
        source_attributes: List[ExpenseAttribute] = ExpenseAttribute.objects.filter(
            ...
            workspace_id=workspace_id,
            attribute_type=source_type,
            mapping__source_id__isnull=True,
        ).all()
            ...
            if destination_attribute.value.lower() in source_value_id_map:
                mapping_batch.append(
                    Mapping(
                        source_type=source_type,
                        destination_type=destination_type,
                        source_id=source_value_id_map[
                            destination_attribute.value.lower()
                        ],
    ...
    @staticmethod
    def auto_map_employees(
        destination_type: str, employee_mapping_preference: str, workspace_id: int
    ):
        '''
        Auto map employees
        ...
        employee_destination_attributes = DestinationAttribute.objects.filter(
            attribute_type=destination_type, workspace_id=workspace_id
        ).all()
        ...
        for destination_employee in employee_destination_attributes:
            ...
            if (
                ...
                and destination_employee.detail
    ...
    @staticmethod
    def auto_map_ccc_employees(
        destination_type: str, default_ccc_account_id: str, workspace_id: int
    ):
        '''
...
class EmployeeMapping(models.Model):
    ...
    destination_employee = models.ForeignKey(
        ...
        on_delete=models.PROTECT,
        null=True,
        related_name='destination_employee',
    )
    destination_vendor = models.ForeignKey(
        DestinationAttribute,
        on_delete=models.PROTECT,
        null=True,
        related_name='destination_vendor',
    )
    destination_card_account = models.ForeignKey(
        DestinationAttribute,
        on_delete=models.PROTECT,
        null=True,
        related_name='destination_card_account',
    )
    workspace = models.ForeignKey(
        Workspace, on_delete=models.PROTECT, help_text='Reference to Workspace model'
    )
    ...
    def create_or_update_employee_mapping(
        ...
        workspace: Workspace,
        destination_employee_id: int = None,
        destination_vendor_id: int = None,
...
class CategoryMapping(models.Model):
    ...
    destination_expense_head = models.ForeignKey(
        ...
        on_delete=models.PROTECT,
        null=True,
        related_name='destination_expense_head',
    )
    workspace = models.ForeignKey(
        Workspace, on_delete=models.PROTECT, help_text='Reference to Workspace model'
    )
    ...
    def create_or_update_category_mapping(
        ...
        destination_expense_head_id: int = None,
    ):
        '''
    ...
    def bulk_create_mappings(
        ...
        source_attributes_id_map = {
            source_attribute['value'].lower(): source_attribute['id']
            for source_attribute in source_attributes
        }
        ...
        for destination_attribute in destination_attributes:
            if destination_attribute.value.lower() in source_attributes_id_map:
                ...
                if destination_type in ('EXPENSE_TYPE', 'EXPENSE_CATEGORY'):
                    destination['destination_expense_head_id'] = (
                ...
                mapping_creation_batch.append(
                    CategoryMapping(
                        source_category_id=source_attributes_id_map[
                            destination_attribute.value.lower()
                        ],
                        workspace_id=workspace_id,
                        **destination
        ...
        return create_mappings_and_update_flag(
            mapping_creation_batch, set_auto_mapped_flag, model_type=CategoryMapping
        )
    ...
    def bulk_create_ccc_category_mappings(workspace_id: int):
        ...
        for category_mapping in category_mappings:
            if (
                category_mapping.destination_expense_head.detail
                and 'gl_account_no' in category_mapping.destination_expense_head.detail
                and category_mapping.destination_expense_head.detail['gl_account_no']
            ):
                ...
                category_mapping.destination_expense_head.detail
                and 'account_internal_id'
                in category_mapping.destination_expense_head.detail
                and category_mapping.destination_expense_head.detail[
                    'account_internal_id'
                ]
            ):
                destination_account_internal_ids.append(
        ...
        for category_mapping in category_mappings:
            ...
            if (
                ...
                in destination_id_pk_map
            ):
                ccc_account_id = destination_id_pk_map[
                    category_mapping.destination_expense_head.detail[
                        'gl_account_no'
                    ].lower()
                ]
